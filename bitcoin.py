...
def computeDelta(wt, X, Xi):
    ...
    c = wt
    N = len(X)-1
    meanX, stdX, pr, nr, dr = np.mean(X[:-1]), np.std(X[:-1]), 0.0, 0.0, 0.0
    
    for row_no, xtemp in Xi.iterrows() :
        yi, stdXI, meanXI, temp = xtemp[-1], np.std(xtemp[:-1]), np.mean(xtemp[:-1]), 0.0
        for i in range(0,N):
            temp += (X[i] - meanX) * (xtemp[i] - meanXI)
        similarity = temp/(N * stdX * stdXI)
